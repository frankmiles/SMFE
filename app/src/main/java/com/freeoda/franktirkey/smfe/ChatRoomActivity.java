  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    channel.addJoinListener(new AsyncCallback<Void>() {
      ...
      public void handleResponse(Void response) {
        Backendless.Messaging.publish(ApplicationClass.DEFAULT_CHANNEL, wrapToColor(name) + " joined", new AsyncCallback<MessageStatus>() {
          @Override
    ...
    message.setOnEditorActionListener(new TextView.OnEditorActionListener() {
      ...
      public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
        ...
        if ( actionId == EditorInfo.IME_ACTION_SEND || event.getKeyCode() == KeyEvent.KEYCODE_ENTER ) {
          ...
          Backendless.Messaging.publish(ApplicationClass.DEFAULT_CHANNEL, wrapToColor("[" + name + "]") + ": " + message.getText().toString(), new AsyncCallback<MessageStatus>() {
            @Override
            public void handleResponse(MessageStatus response) {
